...
// Só funciona para conta com `is_admin` no banco (ver isAdmin abaixo)
...
const SuperAdmin = {
  ...
  // O papel vem da coluna `is_admin` da tabela users, entregue pelo servidor em
  // /api/auth/sessao. Antes era um claim `role='admin'` no app_metadata do JWT
  // da Supabase.
  //
  // Isto aqui e SO cosmetico: esconder o item de menu. Quem de fato barra o
  // acesso e o exigirAdmin do servidor, e ele nao confia nesta funcao nem no
  // que o navegador diga — cada rota de /api/admin reconsulta o banco.
  isAdmin() {
    return !!(App.user && App.user.is_admin === true);
  },
  ...
  async render(container) {
    ...
    try {
      ...
      // Contratos vem por rota propria, nao pela tabela: /api/admin/contracts
      // lista as colunas uma a uma e NUNCA devolve cloud_key, que e a
      // credencial dos links do inquilino.
      const [contracts, profiles, users] = await Promise.all([
        Api.adminContratos(),
        Api.adminPerfis(),
        Api.adminUsuarios()
      ]);
      ...
      container.innerHTML = `
        <div class="empty-state glass">
          ...
          <p style="font-size: 0.9rem; color: var(--text-muted); font-weight: 400;">
            Confirme que sua conta tem o papel de administrador.
          </p>
        </div>
      `;
    }
